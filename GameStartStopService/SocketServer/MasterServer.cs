...
namespace GameStartStopService.SocketServer
{
    class MasterServer : IDisposable
    {
        ...
        internal void ChangeGame(string GameGUID)
        {
            lock (Connections)
                foreach (KeyValuePair<string, SlaveInfo> Machine in Connections)
                {
                    new Action(() =>
                    {
                        JSONResponse<object> Return = Machine.Value.Connection.TransmitJSONCommand<PlayGameModel, object>(new JSONAction<PlayGameModel>() { ActionName = "ChangeGame", ActionData = new PlayGameModel { GUID = GameGUID } });
                        AttendantConsole.Log($"{Machine} Has stoped");
                    }).Invoke();
                }
        }
